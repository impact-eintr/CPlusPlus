...
void test_call_ctor_directly() {
  ...
  //!	A::A(5);	  				//in VC6 : ctor. this=0013FF60
  //!id=5
  //         dtor. this=0013FF60
  // in GCC : [Error] cannot call constructor 'jj02::A::A' directly
...
// C++/11 alias template
template <typename T> using listPool = list<T, __gnu_cxx::__pool_alloc<T>>;
...
void test_overload_global_new() {
  ...
  //***** 測試時, main() 中的其他測試全都 remark, 獨留本測試 *****
  {
    ...
    string *p = new string(
        "My name is Ace"); // jjhou global new(), 4 	(註：這是 string size)
                           // jjhou global new(), 27	(註：這是
                           // sizeof(Rep)+extra)
    ...
    lst.push_back(1); // jjhou global new(), 	12	(註：每個 node是 12
                      // bytes)
...
void test_delete_and_default_for_new() {
  ...
  //!	delete [] pF;			//[Error] use of deleted function 'static void
  //!jj12::Foo::operator delete [](void*, size_t)'

  //! Goo* p2 = new Goo(7);	//[Error] use of deleted function 'static void*
  //! jj12::Goo::operator new(size_t)'
  //!	delete p2;				//[Error] use of deleted function 'static void
  //!jj12::Goo::operator delete(void*, size_t)'
  Goo *pG = new Goo[10];
  delete[] pG;
}
} // namespace jj12
...
template <typename Alloc> void cookie_test(Alloc alloc, size_t n) {
